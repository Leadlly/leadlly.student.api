...
export const postFeedbackData = async (
  ...
  try {
    ...
    if (!chapter || !topic)
      return next(new CustomError("Chapter and Topic required", 400));
...
export const createPlanner = async (
  ...
  try {
    ...
    const allStudentData: IDataSchema[] = await StudyData.find({
      user: user._id,
    });
    ...
    if (
      standard &&
      schedule &&
      scheduleActions[standard] &&
      scheduleActions[standard][schedule]
    ) {
