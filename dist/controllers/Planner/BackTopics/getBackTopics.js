...
const getBackRevistionTopics = async (studentId, dateOfSubscription) => {
  ...
  const unrevisedLowEfficiency = unrevisedTopics.filter(
    (data) => data.topic.overall_efficiency < 40,
  );
  console.log("1");
  const continuousModerateEfficiency = activeContinuousRevisionTopics.filter(
    (data) =>
      data.topic.overall_efficiency >= 40 && data.topic.overall_efficiency < 60,
  );
  ...
  if (currentWeek === 1 || currentWeek === 2) {
    ...
    const filteredUnrevisedTopics = unrevisedTopics.filter(
      (data) => getWeekNumber(data.updatedAt, new Date()) === referenceWeek,
    );
    console.log("4 week2");
