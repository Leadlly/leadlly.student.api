...
exports.getPlanner =
  exports.updateDailyPlanner =
  exports.createPlanner =
    void 0;
...
const createPlanner = async (req, res, next) => {
  try {
    const user = req.user;
    const backRevisionTopics = await (0,
    getBackTopics_1.getBackRevistionTopics)(
      user._id,
      user.subscription.dateOfActivation,
    );
    const result = await (0, generatePlanner_1.generateWeeklyPlanner)(
      user,
      backRevisionTopics,
    );
    res.status(200).json({
...
const updateDailyPlanner = async (req, res, next) => {
  ...
  const yesterdayIST = (0, moment_timezone_1.default)()
    .tz(timezone)
    .subtract(1, "days")
    .startOf("day");
  ...
  const { dailyContinuousTopics, dailyBackTopics } = (0,
  getDailyTopics_1.getDailyTopics)(continuousRevisionTopics, [], user);
  ...
  const dailyQuestions = await (0, getDailyQuestions_1.getDailyQuestions)(
    (0, moment_timezone_1.default)(todayUTC).format("dddd"),
    todayUTC,
    dailyTopics,
  );
  await plannerModel_1.default.updateOne(
