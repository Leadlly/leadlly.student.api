...
const createPlanner = async (req, res, next) => {
  try {
    const user = req.user;
    const allStudentData = await studentData_1.StudyData.find({
      user: user._id,
    });
    ...
    if (
      standard &&
      schedule &&
      actions_1.scheduleActions[standard] &&
      actions_1.scheduleActions[standard][schedule]
    ) {
    ...
    const generatedPlanner = await (0, generatePlanner_1.generateWeeklyPlanner)(
      "666a01a69461d46dc3c7b5fb",
    );
