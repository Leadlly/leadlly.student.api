...
const generateWeeklyPlanner = async (user, backRevisionTopics) => {
  ...
  const activationDate = (0, moment_1.default)(
    user.subscription.dateOfActivation,
  ).startOf("day");
  // Determine the start date of the planner
  const startDate = activationDate.isSameOrAfter(
    (0, moment_1.default)().startOf("isoWeek"),
  )
  ...
  const days = await Promise.all(
    daysOfWeek.map(async (day, index) => {
      ...
      getDailyTopics_1.getDailyTopics)([], backRevisionTopics, user);
      ...
      dailyQuestions = await (0, getDailyQuestions_1.getDailyQuestions)(
        day,
        date,
        dailyTopics,
      );
