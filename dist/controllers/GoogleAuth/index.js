...
const googleAuth = async (req, res, next) => {
  try {
    ...
    const tokenInfoResponse = await axios_1.default.get(
      `https://www.googleapis.com/oauth2/v3/tokeninfo?access_token=${access_token}`,
    );
    const userInfoResponse = await axios_1.default.get(
    ...
    if (!email) return next(new error_1.CustomError("Email not found", 404));
