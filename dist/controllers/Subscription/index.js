...
const buySubscription = async (req, res, next) => {
  try {
    ...
    if (user.role === "admin" || user.role === "mentor") {
      return next(
        new error_1.CustomError("Subscription is only for students", 400),
      );
    }
    const planId = process.env.RAZORPAY_PLAN_ID;
    if (!planId) {
      return next(
        new error_1.CustomError(
...
const verifySubscription = async (req, res, next) => {
  try {
    ...
    const user = await userModel_1.default.findById(req.user._id);
    if (!user) return next(new error_1.CustomError("User not found", 404)); // Ensure user exists
    const subscription_id = user?.subscription.id;
    const secret = process.env.RAZORPAY_API_SECRET;
    if (!secret)
      return next(
        new error_1.CustomError("Razorpay secret is not defined", 400),
      );
...
const cancelSubscription = async (req, res, next) => {
  try {
    const user = await userModel_1.default.findById(req.user._id);
    if (!user) return next(new error_1.CustomError("User not found", 404));
    const subscriptionId = user.subscription.id;
    if (!subscriptionId || typeof subscriptionId !== "string") {
      return next(
        new error_1.CustomError("Subscription ID is not available", 400),
      );
    }
    ...
    if (gap > refundTime) {
      return next(
        new error_1.CustomError(
          "Cannot cancel subscription after 7 days.",
          400,
        ),
      );
    } else {
      ...
      const subscription =
        await Razorpay_1.default.subscriptions.cancel(subscriptionId);
      //Refund the amount
      const refund = await Razorpay_1.default.payments.refund(
