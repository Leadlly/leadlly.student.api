...
const getAllCourses = async (req, res, next) => {
  try {
    ...
    if (!courses) return next(new error_1.CustomError("Course not found", 404));
    res.status(200).json({
      success: true,
      courses,
    });
...
const getCourse = async (req, res, next) => {
  try {
    ...
    if (!course) return next(new error_1.CustomError("Course not found", 404));
    res.status(201).json({
      message: "Course created successfully",
      course,
    });
