...
const register = async (req, res, next) => {
  try {
    ...
    if (user) return next(new error_1.CustomError("User already exists", 400));
...
const resentOtp = async (req, res, next) => {
  ...
  try {
    ...
    if (!otpRecord) return next(new error_1.CustomError("User not found", 404));
    const OTP = (0, generateOTP_1.default)();
    await producer_1.otpQueue.add("otpVerify", {
      options: {
        email,
    ...
    otpRecord.otp = crypto_1.default
      .createHash("sha256")
      .update(OTP)
      .digest("hex");
...
const otpVerification = async (req, res, next) => {
  try {
    ...
    const otpRecord = await otpModal_1.default.findOne({ email });
    if (!otpRecord) return next(new error_1.CustomError("OTP not found", 404));
    const hashedOtp = crypto_1.default
      .createHash("sha256")
      .update(otp)
      .digest("hex");
...
const login = async (req, res, next) => {
  try {
    ...
    if (!isMatched) return next(new error_1.CustomError("Wrong password", 400));
    (0, setCookie_1.default)({
      user,
      res,
      next,
...
const resetpassword = async (req, res, next) => {
  try {
    ...
    if (!user)
      return next(
        new error_1.CustomError("Your link is expired! Try again", 400),
      );
    ...
    crypto_1.default.pbkdf2(
      ...
      async (err, derivedKey) => {
        if (err) return next(new error_1.CustomError(err.message, 500));
...
const getUser = async (req, res, next) => {
  try {
    ...
    if (!user) return next(new error_1.CustomError("User not found", 400));
    res.status(200).json({
      success: true,
      user,
    });
